...
#BTN_GREEN = 8
#BTN_YELLOW = 16
#BTN_RED = 32 ;This is also the regular fire button
#BTN_BLUE = 64 ;This is also the regular second fire button

;Our Function for handling CD32 pad input
Function ButtonIsDown{buttonNumber}
	Function Return GameB(1) & buttonNumber = buttonNumber
End Function

;Our statement for handling shots
Statement Fire{}

	;These variables are shared with the main game
	Shared Shots(),NextShot,X,Y
...
;A basic ASM function for making sure the value of Y is clamped between 0 and 240
;The parameters are loaded into D0,D1,D2 etc
;The return value is loaded into D0
;Make sure address registers A4-A6 are unchanged at the end, everything else can be changed.
Function YClamp{Y}
		
	;The value of Y is in D0. We want to test if it's less than zero
	Tst.w D0
	
	bgt YGreaterThanZero ;Branch if our input value is greater than 0
	Move.w #0,D0 ;Set to 0 if the input value is less than zero
	bra EndYClamp ;Go to the end
	
	.YGreaterThanZero
	cmp.w #240,D0 ;Compare the value with 240
	blt EndYClamp ;If our value is less than 240, the value is safe so we don't need to do anything further
	Move.w #240,D0 ;otherwise, we want to set the value to 240
		
	.EndYClamp
	
	;Always do this when finishing an ASM function
	AsmExit
...
InitCopList #COPPERLIST_MAIN,44,256,COPPERLISTTYPE,8,48,-4
;44 - starting height from beginning of display 
;256 - height of the display (same as 1942 arcade)
;TYPE - our custom settings
;8 - Use 8 sprites
;48 - Use 48 colours
;-4 - Make space for four copper instructions per line. If the number is positive, it would mean four copper instructions in total
...
;Here, we set the text gradient
;The text for "P2"
DisplayRGB #COPPERLIST_MAIN,11,0,143,174,224
DisplayRGB #COPPERLIST_MAIN,11,3,188,188,188
DisplayRGB #COPPERLIST_MAIN,11,4,143,174,224

;Player one's score
DisplayRGB #COPPERLIST_MAIN,1,8,143,174,224
DisplayRGB #COPPERLIST_MAIN,1,11,188,188,188
DisplayRGB #COPPERLIST_MAIN,1,12,143,174,224

;Reset the colors back to default
DisplayRGB #COPPERLIST_MAIN, 1,17,255,255,255
DisplayRGB #COPPERLIST_MAIN,11,18,206,206,156

;Finally, reset the grey color for the lives at the bottom of the screen
DisplayRGB #COPPERLIST_MAIN,11,248,143,174,224
...
While PlayerIsAlive
	...
	if ButtonIsDown{#BTN_REVERSE}
		X - 2
	endif
	if ButtonIsDown{#BTN_FORWARD}
		X + 2
	endif
	if ButtonIsDown{#BTN_PAUSE}
		End
	endif
